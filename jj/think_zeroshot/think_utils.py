def create_cot_message(qid:str, questions:dict, misconceptions:dict, student_answer:str, student_explanation:str) -> str:
    """
    ...
    for r in df.itertuples():
        ...
        message = create_message_v4(r.QuestionId, questions, misconceptions, r.MC_Answer, r.StudentExplanation)
    ...
    question_text = questions[qid]['QuestionText']
    question_choices = "\n".join([x['Choice'] for x in questions[qid]['Choices']])
    question_answer = [x for x in questions[qid]['Choices'] if x['Correct']][0]
    question_misconceptions = "\n".join([f"{x['LabelLong']}: {x['Explanation']}" for x in misconceptions[qid]])

    system_prompt = """You are an excellent math teacher teaching students of year group 1 to 14.
The detail of your lesson is as follows: The students are given a diagnostic multiple choice question.
After choosing their answer, they are also asked to write an explanation for why they chose their answer.
Consider the question, the choices, the correct answer, the student's choice, and the student's explanation.
It is very important to remember these students can have difficulty with spelling and grammar. First interpret what they are trying to say.
Then give a math focused analysis of your interpretation of the student's explanation.
It is very important to not factor in spelling or grammatic mistakes. Focus on the mathematics.
First check if the student's explanation correctly explains how to get the correct answer.
Next check if the student's explanation has a mathematical misconception. Common misconceptions associated with the question are given. It is important to give this extra thought.
However if the student's explanation does not have enough information to conclude that they understand how to correctly solve the question nor that they have a misconception, then you must conclude that the student's explanation is insufficient.
However it is very important that the student's explanation is an actual explanation and not just an answer or lazily written which would make it insufficient for analysis.
No need to provide the correct way to achieve the answer.

OUTPUT FORMAT:
Interpretation: [Your understanding of their mathematical reasoning]
Analysis: [Your analysis of the student's explanation]
Assessment: [Correct/Misconception/Insufficient]"""
...
Now consider the explanation that the student gave for why they chose the answer:
{student_explanation}
"""
...
questions = {
  ...
  "33474": {
    ...
    "Choices": [
      {
        "Choice": "\\( \\frac{1}{3} \\times \\frac{2}{3} \\)",
        "Correct": True
      },
      {
        "Choice": "\\( \\frac{1}{3}+\\frac{2}{3} \\)",
        "Correct": False
      },
      {
        "Choice": "\\( \\frac{2}{3} \\div \\frac{1}{3} \\)",
        "Correct": False
      },
      {
        "Choice": "\\( \\frac{2}{3}-\\frac{1}{3} \\)",
        "Correct": False
      }
    ]
  },
  ...
  "89443": {
    "QuestionText": "What number belongs in the box?\n\\(\n(-8)-(-5)=\n\\square\\)",
    "Choices": [
      {
        "Choice": "\\( -13 \\)",
        "Correct": False
