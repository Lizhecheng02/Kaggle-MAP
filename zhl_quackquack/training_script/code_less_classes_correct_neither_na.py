...
def load_and_preprocess_data():
    ...
    # Further Filter to Neither:NA or Correct:NA Only
    print(f"Original Train has {len(train)} rows")
    train = train[train["misconception_target"].isin(["Correct:NA", "Neither:NA"])].copy()
    print(f"After filtering to Neither:NA, Correct:NA only, Train has {len(train)} rows") 
...
def main():
    ...
    # --- NEW CODE: compute tokenizer lengths and show top 10 ---
    train_df["token_len"] = train_df["text"].apply(
        lambda x: len(tokenizer.encode(x, truncation=False))
    )
    
    # --- Show top 10 longest entries (with full_target) ---
    top10 = train_df.nlargest(10, "token_len")[["token_len", "full_target", "text"]]
    
    print("\nTop 10 longest tokenized inputs:")
    for i, row in top10.iterrows():
        print(f"{i}: {row['token_len']} tokens | target: {row['full_target']}")
        print(row["text"])
        print("-" * 120)
