...
IMPROVED_PROMPTS_CSV = "./raw_data/improved_prompts_mapping.csv"
QA_CHOICE_MAP_CSV = "./raw_data/question_answer_choice_mapping.csv"
...
def attach_improved_questions(df: pd.DataFrame) -> pd.DataFrame:
    # 期望列：QuestionId, improved_question（或 ImprovedQuestion）
    mp = pd.read_csv(IMPROVED_PROMPTS_CSV)
    # 兼容不同列名大小写
    cols = {c.lower(): c for c in mp.columns}
    iq_col = cols.get("improved_question", cols.get("improvedquestion"))
    if iq_col is None:
        raise ValueError("improved_prompts_mapping.csv 里需要列 'improved_question'")

    df = df.merge(mp[["QuestionId", iq_col]].rename(columns={iq_col: "improved_question"}), on="QuestionId", how="left")
    # 仅这五个题会有 improved_question，其他 NaN 时退回原题
    df["QuestionText_eff"] = df["improved_question"].fillna(df["QuestionText"])
    return df


def attach_choice_strings(df: pd.DataFrame) -> pd.DataFrame:
    """
    从长表 CSV (列为 QuestionId, MC_Answer, Choice)
    构造 MC_Options_str，并与 df 按 QuestionId 左连接。
    """
    cm = pd.read_csv(QA_CHOICE_MAP_CSV)
    ...
    # 校验
    required = ['QuestionId', 'Choice', 'MC_Answer']
    if not all(c in cm.columns for c in required):
        raise ValueError(f"CSV 中缺少必要列: {required}")

    # 清洗
    cm['Choice'] = cm['Choice'].astype(str).str.strip().str.lower()
    cm['MC_Answer'] = cm['MC_Answer'].astype(str).str.strip()

    # 排序：a,b,c,d 优先，其他按字母序
    order = ['a', 'b', 'c', 'd']
    others = sorted(set(cm['Choice']) - set(order))
    cm['_order'] = pd.Categorical(cm['Choice'], categories=order + others, ordered=True)
    cm = cm.sort_values(['QuestionId', '_order'])

    # 拼接选项字符串
    cm['opt_str'] = "(" + cm['Choice'] + ") " + cm['MC_Answer']
    cm = cm.groupby('QuestionId', as_index=False)['opt_str'].apply(lambda x: " ".join(x))
    cm = cm.rename(columns={'opt_str': 'MC_Options_str'})  # 👈 保留关键列名

    # 合并
    return df.merge(cm, on='QuestionId', how='left')
...
#     def qid_specific(x):
#         if x['misconception_target'] != "Neither:NA":
#             return str(x["QuestionId"]) + "|" + x['misconception_target']
#         return x['misconception_target']
...
#     mc_options = (
#         train.groupby("QuestionId")["MC_Answer"]
#         .unique()
#         .reset_index()
#         .rename(columns={"MC_Answer": "MC_Options"})
#     )
#     train = train.merge(mc_options, on="QuestionId", how="left")

#     train = train.merge(answers, on="QuestionId", how="left")

#     def make_split_key(row, le):
#         decoded = le.inverse_transform([row["label"]])[0]
#         if "Neither:NA" not in decoded:
...
def load_and_preprocess_data():
    ...
    # 先统一 MC_Answer（Wrong_fraction -> Wrong_Fraction）
    ...
    # 只在训练时使用去掉 True_/False_ 前缀的标签
    train['misconception_target'] = (
        train['Category'].str.split("_", n=1).str[-1] + ":" + train['Misconception']
    )

    # Correct:NA 做题号区分（避免不同题同名标签冲突）
    def qid_specific(x):
        if x['misconception_target'] != "Neither:NA":
            return str(x["QuestionId"]) + "|" + x['misconception_target']
        return x['misconception_target']
    ...
    # 基于 True_* 统计出每题正确答案（is_correct=1）
    idx = train['Category'].str.startswith("True")
    correct = (
        train[idx].groupby(['QuestionId', 'MC_Answer']).size()
        .reset_index(name='c').sort_values('c', ascending=False)
    ...
    # 合并 TrueAnswer、MC_Options
    train = train.merge(mc_options, on="QuestionId", how="left")
    train = train.merge(answers, on="QuestionId", how="left")

    # 选项格式增强（(a) xxx (b) xxx ...），外部 CSV 提供；若不存在则后续在格式化函数里回退
    train = attach_choice_strings(train)

    # 题干增强：有映射的 5 个题用 improved question 覆盖
    train = attach_improved_questions(train)  # 生成列 QuestionText_eff（无映射时回退到原题）

    # 分层 KFold 的 split_key（Neither:NA 需要按题区分）
    def make_split_key(row, le_):
        decoded = le_.inverse_transform([row["label"]])[0]
        if "Neither:NA" not in decoded:
            return str(row["label"])
        else:
            return f"{row['QuestionId']}_{row['label']}"
...
def format_input_v2(row):
    ...
    return (
        f"Question: {row['QuestionText']}\n"
        f"MC Options: {'|'.join(row['MC_Options'])}\n"
...
def format_input_v3(row):
    qtext = row.get("QuestionText_eff", row["QuestionText"])

    opts_str = row.get("MC_Options_str")
    if isinstance(opts_str, str) and len(opts_str) > 0:
        # print("Has opts_str")
        opts_line = f"Multiple-Choice Options: {opts_str}\n"
    else:
        # print("Not Has opts_str")
        letters = ["a", "b", "c", "d"]
        opts = row.get("MC_Options", [])
        formatted_opts = []
        for i, opt in enumerate(opts):
            letter = letters[i] if i < len(letters) else chr(ord("a") + i)
            formatted_opts.append(f"({letter}) {opt}")
        opts_line = f"Multiple-Choice Options: {' '.join(formatted_opts)}\n"
...
def main():
    ...
    if tokenizer.truncation_side == "right":
        print("Setting truncation_side to left")
    ...
    train_df['text'] = train_df.apply(format_input_v3, axis=1)

    sample_rows = train_df.sample(50, random_state=42)
    for i, row in sample_rows.iterrows():
        print(f"\n--- Random Sample (index {i}) ---")
        print(row["text"])
        print("=" * 60)
