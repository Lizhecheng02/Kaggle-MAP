...
import torch
import os
# Mistral-specific imports removed



def load_llama_tokenizer(model_name):
    """Load Llama tokenizer using AutoTokenizer"""
    tokenizer = AutoTokenizer.from_pretrained(
        model_name,
        use_fast=True,
        trust_remote_code=True
    )
    # Llamaモデルはpadトークンを持たない場合があるので設定
...
def format_input(row):
    ...
    # Llama-3.1-Nemotron用のプロンプトフォーマット
    prompt = (
        f"<|system|>You are an AI assistant specialized in analyzing mathematical misconceptions.<|end|>\n"
        f"<|user|>Analyze the following student response:\n\n"
        f"Question: {row['QuestionText']}\n"
        f"Student Answer: {row['MC_Answer']}\n"
        f"Is this correct?: {status}\n"
        f"Student Explanation: {row['StudentExplanation']}\n\n"
        f"What mathematical misconception does this response demonstrate?<|end|>\n"
        f"<|assistant|>"
    )
    # prompt = (
    #     f"<|system|>You are an AI assistant specialized in analyzing mathematical misconceptions.<|end|>\n"
