"""
Magistral-Small-2506 モデルトレーニングスクリプト
"""
...
# カスタムモジュールのインポート
from config import *
from utils import prepare_correct_answers, format_input, tokenize_dataset, compute_map3, load_mistral_tokenizer
...
class MagistralForSequenceClassification(nn.Module):
    """Magistral-Small-2506モデルを分類タスク用にカスタマイズ"""
...
def main():
    ...
    # Mistral Tekkenトークナイザーを使用
    tokenizer = load_mistral_tokenizer(MODEL_NAME)
    ...
    try:
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            ...
            attn_implementation=ATTENTION_IMPLEMENTATION  # config.pyで設定されたアテンション実装を使用
        )
        # パディングトークンIDを設定
        model.config.pad_token_id = tokenizer.pad_token_id
    except:
        # 失敗した場合はカスタムクラスを使用
        print("Using custom classification head for Magistral-Small-2506...")
        # ベースモデルを読み込む
        ...
        # カスタム分類ヘッドを作成
        model = MagistralForSequenceClassification(MODEL_NAME, n_classes)
