...
# カスタムモジュールのインポート
from config import *
from utils import prepare_correct_answers, format_input, tokenize_dataset, compute_map3, DataCollatorWithPadding
...
class Qwen2VLForSequenceClassification(nn.Module):
    ...
    def __init__(self, model_name, num_labels):
        ...
        # configプロパティを追加（参照をコピー）
        self.config = self.qwen.config
        # num_labelsのみ設定（use_return_dictは読み取り専用のため設定しない）
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None, **kwargs):
...
def main():
    ...
    if USE_LORA:
        ...
        print("Full fine-tuning mode (LoRA disabled)")
        # フルファインチューニングの場合は特別な処理は不要
    ...
    training_args = TrainingArguments(
        ...
        gradient_accumulation_steps=GRADIENT_ACCUMULATION_STEPS,  # config.pyから読み込み
    ...
    # エポックあたりのステップ数を計算
    steps_per_epoch = len(train_ds) // (TRAIN_BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS)  # gradient_accumulation_stepsを使用
    total_steps = steps_per_epoch * EPOCHS
    effective_batch_size = TRAIN_BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS
    print(f"\nDataset statistics:")
    print(f"Training samples: {len(train_ds)}")
    print(f"Validation samples: {len(val_ds)}")
    print(f"Batch size: {TRAIN_BATCH_SIZE} (with gradient accumulation: {effective_batch_size})")
