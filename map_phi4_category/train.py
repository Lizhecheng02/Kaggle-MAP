...
# カスタムモジュールのインポート
from config import *
from utils import prepare_correct_answers, format_input, tokenize_dataset, compute_map3, create_compute_metrics_function
...
class CustomTrainer(Trainer):
    def __init__(self, is_correct_eval=None, le_simplified=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.is_correct_eval = is_correct_eval
        self.le_simplified = le_simplified
    
    def compute_metrics(self, eval_pred):
        if self.is_correct_eval is not None and self.le_simplified is not None:
            return compute_map3_with_reconstruction(
                eval_pred, 
                self.is_correct_eval, 
                self.le_simplified
            )
        else:
...
def main():
    ...
    # Removing True_ and False_ prefixes
    ...
    # Remove <think> from val_df
    #train_df = create_text(train_df, use_think=True)
    #val_df = create_text(val_df, use_think=False)
    ...
    eval_is_correct = val_df['is_correct'].tolist()
    compute_metrics_full = create_compute_metrics_function(eval_is_correct, le)
