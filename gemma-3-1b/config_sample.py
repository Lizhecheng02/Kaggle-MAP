...
# Model configuration
VER = 2
MODEL_NAME = "/kaggle/input/gemma-3-1b"
...
WANDB_PROJECT = "gemma-3-1b-math-misconceptions"
WANDB_RUN_NAME = f"gemma-3-1b-ver{VER}"
WANDB_ENTITY = None  # Set your WandB entity (username or team name) if needed

# Early stopping settings
USE_EARLY_STOPPING = True
EARLY_STOPPING_PATIENCE = 10  # Number of evaluations (not epochs) with no improvement after which training will be stopped
EARLY_STOPPING_THRESHOLD = 0.001  # Minimum change in the monitored metric to qualify as an improvement

# LoRA configuration
LORA_R = 16  # LoRAのランク (4-64の範囲が一般的)
LORA_ALPHA = 32  # LoRAのスケーリングパラメータ (通常はrの1-2倍)
LORA_TARGET_MODULES = ["q_proj", "v_proj", "k_proj", "o_proj", "gate_proj", "up_proj", "down_proj"]  # Gemma用の対象モジュール
LORA_DROPOUT = 0.1  # LoRAのドロップアウト率 (0.0-0.3)
LORA_BIAS = "none"  # バイアスの更新設定 ("none", "all", "lora_only")
LORA_TASK_TYPE = "SEQ_CLS"  # タスクタイプ (SEQ_CLS: 分類タスク)
