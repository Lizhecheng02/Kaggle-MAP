...
def create_ocr_correction_dict(csv_path):
    """
    CSVファイルからOCRエラー修正用の辞書を作成

    Args:
        csv_path: OCRエラー報告CSVファイルのパス

    Returns:
        dict: Original文字列をキー、Suggested文字列を値とする辞書
    """
    try:
        df = pd.read_csv(csv_path)
        correction_dict = dict(zip(df['Original'], df['Suggested']))
...
def fix_ocr_errors(text, correction_dict):
    """
    テキスト内のOCRエラーを修正

    Args:
        text: 修正対象のテキスト
        correction_dict: 置換辞書

    Returns:
        str: OCRエラーが修正されたテキスト
    """
    ...
    # 長い文字列から順に置換（より具体的なパターンを優先）
    sorted_keys = sorted(correction_dict.keys(), key=len, reverse=True)

    for original in sorted_keys:
        suggested = correction_dict[original]
        corrected_text = corrected_text.replace(original, suggested)
