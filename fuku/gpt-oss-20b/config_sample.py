"""
設定ファイル - GPT-OSS-20B モデルのトレーニングと推論用設定
"""

# Model configuration
VER = 2
MODEL_NAME = "/hdd/models/gpt-oss-20b"
MODEL_TYPE = "gpt"  # GPT-OSS-20B model type
EPOCHS = 3  # Reduce epochs for initial testing
MAX_LEN = 250  # GPT-OSS-20B supports 32k context, but we use 1024 for efficiency

# Directory settings
OUTPUT_DIR = f"ver_{VER}"

# Training parameters
TRAIN_BATCH_SIZE = 4  # Smaller batch size for GPT-OSS-20B
...
WANDB_PROJECT = "gpt-oss-20b-math-misconceptions"
WANDB_RUN_NAME = f"gpt-oss-20b-ver{VER}"
...
# LoRA configuration for GPT-OSS-20B
LORA_RANK = 64  # LoRAのランク - optimized for GPT-OSS-20B
LORA_ALPHA = 128  # LoRAのスケーリングパラメータ - 1:1 ratio with rank
LORA_TARGET_MODULES = ["c_attn", "c_proj", "c_fc", "mlp.c_proj"]  # GPT-OSS-20B target modules
LORA_DROPOUT = 0.1  # LoRAのドロップアウト率 - reduced for GPT-OSS-20B
...
BF16 = True  # Use BF16 training for GPT-OSS-20B
