...
# カスタムモジュールのインポート
from config import *
from utils import prepare_correct_answers, format_input, tokenize_dataset, compute_map3, rewrite_questions
...
def main():
    ...
    train = rewrite_questions(train, REWRITE_QUESTION_LIST)
    ...
    # パディングトークンの設定
    # Qwen2.5モデルの場合、特別なトークンIDを使用
    if tokenizer.pad_token is None:
        # 語彙内の安全なトークンIDを使用
        # Qwen2.5モデルでは、EOSトークンをパディングトークンとして使用
