...
def compute_map3(y_true, y_pred_probs, label_encoder):
    ...
    for i in range(n_samples):
        ...
        true_label = y_true[i]
        if top3_labels[0] == true_label:
            score += 1.0
        elif top3_labels[1] == true_label:
            score += 1.0 / 2
        elif top3_labels[2] == true_label:
            score += 1.0 / 3
...
print("Calculating validation MAP@3...")
...
val_probs = np.zeros((n_val, n_classes))

# Assign probabilities based on predictions
for i, pred in enumerate(val_predictions):
    if pred in target_to_idx:
        val_probs[i, target_to_idx[pred]] = 1.0
    else:
        # If prediction not in training targets, assign uniform probability
        val_probs[i] = 1.0 / n_classes
