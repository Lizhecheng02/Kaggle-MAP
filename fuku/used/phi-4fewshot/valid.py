...
# 設定ファイルをインポート
from config import *
from utils import format_input, prepare_correct_answers, build_fewshot_examples_map
...
def prepare_data(label_encoder):
    """トレーニングデータを準備（ラベル列挙 + few-shot対応）"""
    ...
    # few-shot用のマップを作成
    print("Building few-shot examples map...")
    fewshot_map = build_fewshot_examples_map(train_df)
    
    # プロンプトを作成（ラベル列挙 + few-shot）
    print("Formatting prompts with label enumeration and few-shot examples...")
...
def main():
    ...
    # データを準備
    train_df, dataset = prepare_data(label_encoder)
