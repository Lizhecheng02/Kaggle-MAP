...
class Phi4ForSequenceClassification(nn.Module):
    ...
    def __init__(self, model_name, num_labels, attn_implementation="eager", class_weights=None):
        super().__init__()
        from transformers import AutoModel
...
def main():
    ...
    class_distribution = class_counts / len(train)
    
    print(f"Number of classes: {n_classes}")
    print(f"Class counts distribution:")
    for i in range(min(10, len(class_counts))):
        class_name = le.inverse_transform([i])[0]
        count = class_counts.iloc[i]
        percentage = class_distribution.iloc[i] * 100
    ...
    # クラス重みの計算
    if USE_CLASS_WEIGHTS and CLASS_WEIGHT_STRATEGY == "balanced":
        ...
        print(f"Using balanced class weights (first 10): {class_weights[:10]}")
    else:
        class_weights = None
    ...
    try:
        ...
        # カスタム分類ヘッドを作成（クラス重み付きで）
        model = Phi4ForSequenceClassification(MODEL_NAME, n_classes, ATTENTION_IMPLEMENTATION, class_weights)
