...
def format_input(row):
    """入力データをQwQ-32B用プロンプトにフォーマット"""
    if row["is_correct"]:
        status = "Yes"
    else:
        status = "No"

    # QwQ-32B用の数学推論タスクに特化したプロンプト
    prompt = (
        "<|im_start|>user\n"
        f"Analyze this student's mathematical reasoning and identify the specific misconception or error category.\n\n"
        f"Question: {row['QuestionText']}\n"
        f"Student's Answer: {row['MC_Answer']}\n"
        f"Is Answer Correct?: {status}\n"
