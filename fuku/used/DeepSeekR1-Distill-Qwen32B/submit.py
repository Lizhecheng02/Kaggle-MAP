"""
DeepSeek-R1-Distill-Qwen-32B モデル推論スクリプト（bitsandbytes 4bit版） - 提出用予測ファイルの生成
"""
...
def main():
    ...
    if PEFT_AVAILABLE:
        ...
        # BitsAndBytes 4bit量子化設定
        if USE_4BIT_QUANTIZATION:
            print("Setting up BitsAndBytes 4-bit quantization for inference...")
        ...
        # DeepSeek-R1モデルを量子化で読み込み
        model = AutoModelForSequenceClassification.from_pretrained(
            MODEL_NAME,
            num_labels=n_classes,
            trust_remote_code=True,
            quantization_config=quantization_config,  # BitsAndBytes設定
        ...
        print("Successfully loaded LoRA fine-tuned DeepSeek-R1 model with BitsAndBytes 4-bit quantization")
    else:
        # PEFTが利用できない場合はエラー
