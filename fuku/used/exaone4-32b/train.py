...
class ExaoneForSequenceClassification(nn.Module):
    """EXAONEモデルを分類タスク用にカスタマイズ"""
    def __init__(self, model_name, num_labels, quantization_config=None):
        super().__init__()
        from transformers import AutoModel
        if quantization_config is not None:
            self.exaone = AutoModel.from_pretrained(
                model_name, 
            ...
            self.exaone = AutoModel.from_pretrained(model_name, trust_remote_code=True)
        self.dropout = nn.Dropout(0.1)
        self.classifier = nn.Linear(self.exaone.config.hidden_size, num_labels)

    def forward(self, input_ids, attention_mask=None, labels=None):
        outputs = self.exaone(input_ids=input_ids, attention_mask=attention_mask)
        # 最後のトークンの隠れ状態を使用
...
def main():
    ...
    # パディングトークンの設定
    # EXAONEモデルの場合のパディングトークン設定
    if tokenizer.pad_token is None:
        # EXAONEモデルでは通常EOSトークンをパディングトークンとして使用
    ...
    try:
        ...
        if USE_4BIT_QUANTIZATION:
            ...
            # 通常精度でモデルを読み込む（bfloat16で計算）
            model = AutoModelForSequenceClassification.from_pretrained(
                MODEL_NAME,
                num_labels=n_classes,
                trust_remote_code=True,
                torch_dtype=torch.bfloat16,  # bfloat16精度で読み込む
                device_map="auto",  # 自動的にGPUに配置
                low_cpu_mem_usage=True  # CPUメモリ使用量を削減
            )
        # パディングトークンIDを設定
        ...
        # 失敗した場合はカスタムクラスを使用
        print("Using custom classification head for EXAONE...")
        # カスタム分類ヘッドを作成（量子化設定を渡す）
        model = ExaoneForSequenceClassification(MODEL_NAME, n_classes, quantization_config)
