...
def main():
    ...
    print(f"  USE_8BIT_ADAM: {USE_8BIT_ADAM}")
    print(f"  MAX_GRAD_NORM: {MAX_GRAD_NORM}")
    print(f"  4-bit quantization: ENABLED (load_in_4bit=True)")
        ...
        class GemmaForSequenceClassificationOptimized(nn.Module):
            def __init__(self, model_name, num_labels):
                ...
                self.gemma = AutoModel.from_pretrained(
                    ...
                    quantization_config=quantization_config,  # 4bit量子化を使用
        ...
        if os.path.exists(safetensors_path):
            with safe_open(safetensors_path, framework='pt') as f:
                ...
                if classifier_weight_key in f.keys():
                    ...
                    model.classifier.weight.data = model.classifier.weight.data.to(torch.float16)
                    if model.classifier.bias is not None:
                        model.classifier.bias.data = model.classifier.bias.data.to(torch.float16)
