...
# Model configuration
VER = 2
MODEL_NAME = "/kaggle/input/Qwen2.5-0.5B-Instruct"  # Qwen2.5-14B-Instruct model from HuggingFace
MODEL_TYPE = "qwen"  # Qwen-2.5-14B model type
EPOCHS = 3  # Reduce epochs for initial testing
MAX_LEN = 250  # Qwen-2.5-14B supports longer context

# Directory settings
OUTPUT_DIR = f"ver_{VER}"

# Training parameters
TRAIN_BATCH_SIZE = 4  # Smaller batch size for 14B model
...
# LoRA configuration for Qwen2.5-14B
LORA_RANK = 32  # LoRAのランク - reduced for 14B model
LORA_ALPHA = 64  # LoRAのスケーリングパラメータ - 2:1 ratio with rank
LORA_TARGET_MODULES = ["q_proj", "v_proj", "k_proj", "o_proj", "gate_proj", "up_proj", "down_proj"]  # Qwen2.5 target modules
