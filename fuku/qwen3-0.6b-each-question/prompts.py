...
def create_prompt_v2(tokenizer, row):
    """詳細なプロンプト生成（v2）- 質問の選択肢と誤解の説明を含む"""
    if row["is_correct"]:
        status = "Yes"
    else:
        status = "No"

    # Get question information if available
    question_id = row.get('QuestionId', '')
    choices = ""
    misconception_text = ""
    label_choices = ""

    if question_id in question_choices:
        for c in question_choices[question_id]:
            choices += f"• {c}\n"

    if question_id in misconception_explanations:
        for m in misconception_explanations[question_id]:
            misconception_text += f"{m['Label']} ({m['LabelLong']}): {m['Explanation']}\n"

    if question_id in question_label_choices:
        for c in question_label_choices[question_id]:
            label_choices += f"• {c}\n"
...
def create_prompt_original(tokenizer, row):
    """元のプロンプト生成方式（後方互換性のため）"""
    if row["is_correct"]:
        status = "Yes"
    else:
        status = "No"

    # Qwen3モデル用のシステム・ユーザーメッセージ形式
    messages = [
        {
            "role": "system",
            "content": (
...
prompt_registry = {
    ...
    "create_prompt_original": create_prompt_original,
}