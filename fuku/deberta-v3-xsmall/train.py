...
from transformers import DebertaV2ForSequenceClassification, DebertaV2Tokenizer, TrainingArguments, Trainer, get_cosine_schedule_with_warmup
...
def main():
    ...
    if USE_DATA_AUGMENTATION:
        ...
        train_ds = augment_dataset(train_ds, tokenizer, AUGMENTATION_CONFIG)
    ...
    training_args = TrainingArguments(
        ...
        report_to="none",
        warmup_ratio=WARMUP_RATIO if USE_COSINE_SCHEDULER else 0.0,
    ...
    # カスタムオプティマイザとスケジューラーの設定
    if USE_COSINE_SCHEDULER:
        # 学習率スケジューラーのステップ数を計算
        num_training_steps = len(train_ds) // TRAIN_BATCH_SIZE * EPOCHS
