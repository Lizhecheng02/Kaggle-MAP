...
def main():
    ...
    print(f"Original train shape: {train_all.shape}")
    
    # 全データで前処理（LabelEncoderのため）
    train_all.Misconception = train_all.Misconception.fillna('NA')
    train_all['target'] = train_all.Category + ":" + train_all.Misconception
    train_all['label'] = le.fit_transform(train_all['target'])
    n_classes = len(le.classes_)
    print(f"Number of unique targets: {n_classes}")
    
    # 学習データはQuestionID 31772のみ
    train = train_all[train_all['QuestionId'] == 31772].copy()
    print(f"Filtered train shape (QuestionID 31772 only): {train.shape}")
    
    # 検証データ用に全データを保持
    val_candidates = train_all.copy()

    # --- 特徴量エンジニアリング ---
    print("Performing feature engineering...")
    # 学習データ用
    correct_train = prepare_correct_answers(train)
    train = train.merge(correct_train, on=['QuestionId','MC_Answer'], how='left')
    train.is_correct = train.is_correct.fillna(0)
    
    # 検証データ用（全データ）
    correct_val = prepare_correct_answers(val_candidates)
    val_candidates = val_candidates.merge(correct_val, on=['QuestionId','MC_Answer'], how='left')
    val_candidates.is_correct = val_candidates.is_correct.fillna(0)

    # --- 入力テキストのフォーマット ---
    print("Formatting input text...")
    train['text'] = train.apply(format_input, axis=1)
    val_candidates['text'] = val_candidates.apply(format_input, axis=1)
    ...
    # 検証データは全データから分割
    _, val_df = train_test_split(val_candidates, test_size=VALIDATION_SPLIT, random_state=RANDOM_SEED)
    # 学習データはQuestionID 31772のみ使用
    train_df = train
    
    print(f"Train set size (QuestionID 31772 only): {len(train_df)}")
    print(f"Validation set size (all QuestionIDs): {len(val_df)}")
    print(f"Validation set unique QuestionIDs: {val_df['QuestionId'].nunique()}")
