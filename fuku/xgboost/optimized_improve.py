...
if __name__ == '__main__':
    ...
    for config_idx, xgb_params in enumerate(model_configs):
        print(f"\nTraining model configuration {config_idx + 1}")
        ...
        for fold, (train_idx, valid_idx) in enumerate(skf.split(X_train_all, train['target_encoded'])):
            ...
            # 学習率スケジューラーのコールバック設定
            # XGBoostの組み込みLearning Rate Schedulerを使用
            callbacks = [
                xgb.callback.EarlyStopping(rounds=100, save_best=True),
                xgb.callback.LearningRateScheduler(
                    lambda epoch: xgb_params['learning_rate'] * (0.99 ** (epoch // 100))
                )
            ]
