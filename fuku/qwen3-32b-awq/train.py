    ...
    def __init__(self, model_name, num_labels, quantization_config=None):
        ...
        if quantization_config is not None:
            self.qwen = AutoModel.from_pretrained(
                ...
                device_map="cuda:0",  # 明示的に単一GPUに配置
                low_cpu_mem_usage=True
            )
        else:
            self.qwen = AutoModel.from_pretrained(model_name, trust_remote_code=True, device_map="cuda:0")
...
def main():
    ...
    if USE_AWQ_QUANTIZATION:
        print("Setting up AWQ quantization...")
        quantization_config = AwqConfig(
            bits=4,
            do_fuse=AWQ_FUSE_LAYERS,
            fuse_max_seq_len=AWQ_FUSE_MAX_SEQ_LEN if AWQ_FUSE_LAYERS else None
        )
        print(f"AWQ quantization enabled (fused layers: {AWQ_FUSE_LAYERS})")

    try:
        # AWQモデルを読み込む（量子化設定は自動検出）
        print(f"Loading AWQ model from {MODEL_NAME}...")
        model = AutoModelForSequenceClassification.from_pretrained(
            MODEL_NAME,
            num_labels=n_classes,
            trust_remote_code=True,
            quantization_config=quantization_config,  # AWQ用の設定
        ...
        # 失敗した場合はカスタムクラスを使用
        print("Using custom classification head for Qwen3...")
        # カスタム分類ヘッドを作成（量子化設定を渡す）
        model = Qwen2ForSequenceClassification(MODEL_NAME, n_classes, quantization_config)
    ...
    # AWQモデルはすでに量子化済みのため、prepare_model_for_kbit_trainingは不要
    # ただし、gradient checkpointingを有効化
    ...
    # 訓練可能なパラメータ（LoRAウェイト）をFP32にキャスト
    # AWQ量子化モデルとfp16+gradient clippingを併用する際に必要
    print("Casting trainable parameters to FP32 for gradient clipping compatibility...")
    ...
    training_args = TrainingArguments(
        ...
        bf16=False,  # AWQ使用時は混合精度を無効化
        fp16=True,   # AWQでは通常fp16が推奨
