...
class DataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        for key in features[0].keys():
            if key in ("label", "label_cat", "label_mc"):
                # ラベルはパディング不要
                if key in features[0]:
                    batch[key] = torch.tensor([int(f[key]) for f in features], dtype=torch.long)
        ...
        if "label" in batch:
            batch["labels"] = batch.pop("label")  # label→labels
        # 階層タスク用ラベルをモデルの引数名に合わせて変更
        if "label_cat" in batch:
            batch["labels_cat"] = batch.pop("label_cat")
        if "label_mc" in batch:
            batch["labels_mc"] = batch.pop("label_mc")
